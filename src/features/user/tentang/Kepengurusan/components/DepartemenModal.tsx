...
/** Pisah ketua/wakil dari anggota biasa */
function isLeader(jabatan: string) {
    const j = jabatan.toLowerCase();
    return j.includes("ketua") || j.includes("wakil");
}

export default function DepartemenModal({ open, onClose, namaDepartemen, img, anggota }: Props) {
    ...
    return (
        <div
            ...
            className={`fixed inset-0 z-[999] flex items-end sm:items-center justify-center transition-all duration-300
                ${open ? "bg-black/75 backdrop-blur-sm" : "bg-transparent pointer-events-none"}`}
            aria-modal="true"
            role="dialog"
            aria-label={`Anggota ${namaDepartemen}`}
        >
            {/* Panel */}
            <div
                className={`relative w-full sm:max-w-lg bg-[#0d0d0d] border border-[#2a2a2a]
                ...
                <div className="relative shrink-0">
                    {img ? (
                        ...
                        <div className="px-5 pt-5 pb-4 border-b border-[#1e1e1e]">
                            <p className="text-[#ffd700] text-[10px] uppercase tracking-[2px] mb-1">Departemen</p>
                            <h2 className="text-white font-bold text-lg">{namaDepartemen}</h2>
                    ...
                    <button
                        onClick={onClose}
                        className="absolute top-3 right-3 w-8 h-8 sm:w-9 sm:h-9 bg-black/60 backdrop-blur-sm border border-white/10 flex items-center justify-center hover:bg-black/80 transition-colors"
                        aria-label="Tutup"
                    >
                ...
                <div className="overflow-y-auto overscroll-contain flex-1 px-4 sm:px-5 py-4 space-y-4">
                    ...
                    {leaders.length > 0 && (
                        <div className="space-y-2">
                            ...
                            {leaders.map((m, i) => (
                                <MemberRow key={i} member={m} isLeader />
                            ))}
                        </div>
                    )}
...
/* ── Row anggota dalam modal ── */
function MemberRow({ member, isLeader = false }: { member: { nama: string; jabatan: string; image?: string | null }; isLeader?: boolean }) {
    return (
        ...
        >
            ...
            {/* Indikator leader */}
            {isLeader && (
                <Crown size={12} className="text-[#ffd700]/60 shrink-0" />
            )}
        </div>
    );
}
